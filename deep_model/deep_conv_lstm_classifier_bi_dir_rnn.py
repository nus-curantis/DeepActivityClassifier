...
class DeepConvLSTMClassifier:
    ...
    def build_model(self):
        ...
        with tf.name_scope('rnn'):
            ...
            self.rnn_output, _ = tf.nn.bidirectional_dynamic_rnn(
                cell_fw=self.rnn_cell, cell_bw=self.rnn_cell, inputs=self.embedded_input,
                dtype=tf.float32, sequence_length=self.__length(self.embedded_input))

            print('********************************************************************rnn_output', self.rnn_output)
            print('rnn_output', self.rnn_output[0])
            print('rnn_output', self.rnn_output[1])

            rnn_output_concatenated = tf.concat(
                [self.rnn_output[0], self.rnn_output[1]], axis=2
            )

            print('rnn_output_concatenated', rnn_output_concatenated)

        with tf.name_scope('time_distributed_layer'):
            rnn_output_reshaped = tf.reshape(rnn_output_concatenated, shape=[-1, 2 * self.rnn_hidden_units])

            self.time_distributed_w = tf.Variable(tf.truncated_normal([2 * self.rnn_hidden_units,
                                                                       2 * self.rnn_hidden_units]))
            self.time_distributed_b = tf.Variable(tf.zeros([2 * self.rnn_hidden_units]))

            time_distributed_output = tf.matmul(rnn_output_reshaped, self.time_distributed_w) + self.time_distributed_b
            self.time_distributed_output = tf.reshape(time_distributed_output, shape=tf.shape(rnn_output_concatenated))
        ...
        with tf.name_scope('pooling'):
            ...
            print('self.time_distributed_output', self.time_distributed_output)
        ...
        with tf.name_scope('predictor'):
            self.dense_weights = {
                'first': tf.Variable(tf.truncated_normal(
                    [6 * self.rnn_hidden_units, 4 * self.rnn_hidden_units])),
                'second': tf.Variable(tf.truncated_normal(
                    [4 * self.rnn_hidden_units, 2 * self.rnn_hidden_units])),
                'third': tf.Variable(tf.truncated_normal([2 * self.rnn_hidden_units, self.num_classes]))
            }

            self.dense_biases = {
                'first': tf.Variable(tf.zeros([4 * self.rnn_hidden_units])),
        ...
        self.avg_pooling_summary = tf.summary.image(
            'avg pooling', tf.reshape(self.avg_pooling, shape=[-1, 2 * self.rnn_hidden_units, 1, 1]))
        self.max_pooling_summary = tf.summary.image(
            'max pooling', tf.reshape(self.max_pooling, shape=[-1, 2 * self.rnn_hidden_units, 1, 1]))
        self.last_pooling_summary = tf.summary.image(
            'mean pooling', tf.reshape(self.last_pooling, shape=[-1, 2 * self.rnn_hidden_units, 1, 1]))
